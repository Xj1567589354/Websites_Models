$(function(){
    ...
    var index=1;
    var url="http://119.29.61.46/PHP/CoverImg.php?type=bg"+"&r=";
 ...
 function SreenFit(){
     ...
     //获取一个虚拟img
        $('<img/>').attr('src',backgroundimg.src).load(function(){
            if($('section').width()>this.width||$('.body').height()>this.height)
            {
                if($('section').width()>this.width)
                {
                ...
                $('section').css('background-size','auto auto');
            }
        });
    };
    //实时传入遮罩透明度
    var FadeSet=function(op){
        $('section').css('background-image','linear-gradient(rgba(0,0,0,'+op+'),rgba(0,0,0,'+op+')),url('+backgroundimg.src+')');
    };
    //设置透明度
